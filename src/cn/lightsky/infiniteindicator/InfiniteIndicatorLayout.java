...
/**
 * Created by lightSky on 2014/12/22. Thanks to:
...
public class InfiniteIndicatorLayout extends RelativeLayout implements
	...
	public InfiniteIndicatorLayout(Context context, AttributeSet attrs,
		...
		final TypedArray attributes = context.obtainStyledAttributes(attrs,
				R.styleable.InfiniteIndicatorLayout, 0, 0);
		int indicatorType = attributes.getInt(
				R.styleable.InfiniteIndicatorLayout_indicator_type,
				IndicatorType.Default.ordinal());

		if (indicatorType == 0)
			LayoutInflater.from(context).inflate(
					R.layout.layout_default_indicator, this, true);
		else if (indicatorType == 1)
			LayoutInflater.from(context).inflate(
					R.layout.layout_anim_circle_indicator, this, true);
		else
			LayoutInflater.from(context).inflate(
					R.layout.layout_anim_line_indicator, this, true);
	...
	private List<BaseSliderView> views = null;

	public <T extends BaseSliderView> void addSlider(T imageContent) {

		if (null == views) {
			views = new ArrayList<BaseSliderView>();
		}

		mRecyleAdapter.addSlider(imageContent);

		views.add(imageContent);
	...
	public void start() {
		...
		for (BaseSliderView imgContent : views) {
			mRecyleAdapter.removeSlider(imgContent);
		}
	...
	public static class ScrollHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			InfiniteIndicatorLayout infiniteIndicatorLayout = mLeakActivityRef
					.get();
	...
	/**
	 * get auto scroll interval time in milliseconds, default is
	...
	/**
	 * set auto scroll interval time in milliseconds, default is
	 * {@link #DEFAULT_INTERVAL}
	 * 
	 * @param interval
	 *            the interval to set
	...
	/**
	 * set how to process when sliding at the last or first item will be explore
	 * in future version
	 * 
	...
	public enum IndicatorPosition {
		Center("Center_Bottom", R.id.default_center_indicator), Center_Bottom(
				"Center_Bottom", R.id.default_center_bottom_indicator), Right_Bottom(
	...
	public void setIndicatorPosition(IndicatorPosition presentIndicator) {
		PageIndicator pagerIndicator = (PageIndicator) findViewById(presentIndicator
				.getResourceId());
